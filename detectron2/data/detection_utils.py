...
def annotations_to_instances_rotated(annos, image_size):
    ...
    classes = torch.tensor(classes, dtype=torch.int64)
    target.gt_classes = classes
    target.key_lpoint = torch.as_tensor([obj["key_lpoint"] for obj in annos])
    target.key_rpoint = torch.as_tensor([obj["key_rpoint"] for obj in annos])
    target.inflection_point = torch.as_tensor([obj["inflection_point"] for obj in annos])
    target.bbox_lr = torch.as_tensor([obj["bbox_lr"] for obj in annos])
