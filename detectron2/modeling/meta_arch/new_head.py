...
from .kp_utils import _decode_box
...
class Centripetal_mask(nn.Module):
    ...
    def _init_layers(self):
        # self.prefadp = DeformConv(self.in_channels, self.in_channels, 3, 1, 1)
        # self.pre_offset = nn.Conv2d(2, 18, 1, bias=False)
        # self.pre_off_c = make_kp_layer(out_dim=2)

        self.p2_fadp = DeformConv(self.in_channels, self.in_channels, 3, 1, 1)
        # self.br_fadp = DeformConv(self.in_channels, self.in_channels, 3, 1, 1)
        self.mid_p2_fadp = DeformConv(self.in_channels, self.in_channels, 3, 1, 1)
        # self.mid_br_fadp = DeformConv(self.in_channels, self.in_channels, 3, 1, 1)

        self.p2_offset = nn.Conv2d(2, 18, 1, bias=False)
        # self.br_offset = nn.Conv2d(2, 18, 1, bias=False)
        self.mid_p2_offset = nn.Conv2d(2, 18, 1, bias=False)
        ...
        self.p2_heat = make_kp_layer(out_dim=self.num_classes)
        self.p1_heat = make_kp_layer(out_dim=self.num_classes)


        self.tl_det = make_kp_layer(out_dim=self.num_classes)
        self.br_det = make_kp_layer(out_dim=self.num_classes)
        self.mid_tl_det = make_kp_layer(out_dim=self.num_classes)
        self.mid_br_det = make_kp_layer(out_dim=self.num_classes)
        self.p2_off_c = make_kp_layer(out_dim=2)
        # self.br_off_c = make_kp_layer(out_dim=2)


        self.p2_off_c_2 = make_kp_layer(out_dim=2)
        # self.br_off_c_2 = make_kp_layer(out_dim=2)

        self.tl_off = make_kp_layer(out_dim=2)
        self.br_off = make_kp_layer(out_dim=2)
        self.p1_off = make_kp_layer(out_dim=2)

        # middle supervision

        self.mid_p2_heat = make_kp_layer(out_dim=self.num_classes)
        self.mid_p1_heat = make_kp_layer(out_dim=self.num_classes)

        self.mid_p2_off_c = make_kp_layer(out_dim=2)
        # self.mid_br_off_c = make_kp_layer(out_dim=2)

        self.mid_p2_off_c_2 = make_kp_layer(out_dim=2)
    ...
    def init_weights(self):
        ...
        """
        self.p2_heat[-1].bias.data.fill_(-2.19)
        self.p1_heat[-1].bias.data.fill_(-2.19)
        self.mid_p2_heat[-1].bias.data.fill_(-2.19)
        self.mid_p1_heat[-1].bias.data.fill_(-2.19)
        normal_init(self.p2_offset, std=0.1)
        normal_init(self.pre_offset, std=0.1)
        normal_init(self.p2_fadp, std=0.01)
        # normal_init(self.br_offset, std=0.1)
        # normal_init(self.br_fadp, std=0.01)
        normal_init(self.mid_p2_offset, std=0.1)
        normal_init(self.mid_p2_fadp, std=0.01)
    ...
    def forward_single(self, feats):
        ...
        p2_pool = x
        p2_heat = self.p2_heat(x)
        tl_off_c = self.p2_off_c(p2_pool)  # 可变形卷积offset
        # tl_off = self.tl_off(tl_pool)

        tl_off_c_d = torch.zeros(tl_off_c.size())
        tl_off_c_d[:, 0, :, :] = tl_off_c[:, 0, :, :] * 10
        # br_off_c[:, 0, :, :] *= 10
        tl_off_c_d[:, 1, :, :] = tl_off_c[:, 1, :, :]
        # br_off_c[:, 1, :, :] *= 4
        tl_off_c_d[:, 1, :, :] = torch.clamp(tl_off_c_d[:, 1, :, :], min=0.0)
        # br_off_c[:, 1, :, :] = torch.clamp(br_off_c[:, 1, :, :], min=0.0)
        tl_off_c2 = torch.zeros(tl_off_c.size())
        # br_off_c2 = torch.zeros(tl_off_c.size())
        tl_off_c2[:, 0, :, :] = torch.cos(tl_off_c_d[:, 0, :, :] / 180 * np.pi) * tl_off_c_d[:, 1, :, :]
        tl_off_c2[:, 1, :, :] = -torch.sin(tl_off_c_d[:, 0, :, :] / 180 * np.pi) * tl_off_c_d[:, 1, :, :]
        # br_off_c2[:, 0, :, :] = torch.cos(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        # br_off_c2[:, 1, :, :] = -torch.sin(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        tl_off_c2 = tl_off_c2.to("cuda")

        p2_offmap = self.p2_offset(tl_off_c2.detach())  # 到可变形卷积的各个点
        x_p2_fadp = self.p2_fadp(p2_pool, p2_offmap)
        tl_det = self.tl_det(x_p2_fadp)
        br_det = self.br_det(x_p2_fadp)
        tl_off = self.tl_off(x_p2_fadp)
        br_off = self.br_off(x_p2_fadp)
        p2_off_c_2 = self.p2_off_c_2(x_p2_fadp)  # 矢量

        p1_pool = x
        p1_heat = self.p1_heat(x)
        # br_off_c = self.br_off_c(br_pool)
        p1_off = self.p1_off(p1_pool)

        # br_off_c_d = torch.zeros(br_off_c.size())
        # br_off_c_d[:, 0, :, :] = br_off_c[:, 0, :, :] * 10
        # # br_off_c[:, 0, :, :] *= 10
        # br_off_c_d[:, 1, :, :] = br_off_c[:, 1, :, :]
        # # br_off_c[:, 1, :, :] *= 4
        # br_off_c_d[:, 1, :, :] = torch.clamp(br_off_c_d[:, 1, :, :], min=0.0)
        # # br_off_c[:, 1, :, :] = torch.clamp(br_off_c[:, 1, :, :], min=0.0)
        # br_off_c2 = torch.zeros(tl_off_c.size())
        # # br_off_c2 = torch.zeros(tl_off_c.size())
        # br_off_c2[:, 0, :, :] = torch.cos(br_off_c_d[:, 0, :, :] / 180 * np.pi) * br_off_c_d[:, 1, :, :]
        # br_off_c2[:, 1, :, :] = -torch.sin(br_off_c_d[:, 0, :, :] / 180 * np.pi) * br_off_c_d[:, 1, :, :]
        # # br_off_c2[:, 0, :, :] = torch.cos(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        # # br_off_c2[:, 1, :, :] = -torch.sin(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        # br_off_c2 = br_off_c2.to("cuda")
        #
        # br_offmap = self.br_offset(br_off_c2.detach())
        # x_br_fadp = self.br_fadp(br_pool, br_offmap)
        # br_off_c_2 = self.br_off_c_2(x_br_fadp)

        p2_result = torch.cat([p2_heat, tl_off_c, p2_off_c_2, tl_det, br_det, tl_off, br_off], 1)
        p1_result = torch.cat([p1_heat, p1_off], 1)
        ...
        p2_pool_mid = x
        p2_heat_mid = self.mid_p2_heat(x)
        tl_off_c_mid = self.mid_p2_off_c(p2_pool_mid)  # 可变形卷积offset
        # tl_off = self.tl_off(tl_pool)

        tl_off_c_d_mid = torch.zeros(tl_off_c_mid.size())
        tl_off_c_d_mid[:, 0, :, :] = tl_off_c_mid[:, 0, :, :] * 10
        # br_off_c[:, 0, :, :] *= 10
        tl_off_c_d_mid[:, 1, :, :] = tl_off_c_mid[:, 1, :, :]
        # br_off_c[:, 1, :, :] *= 4
        tl_off_c_d_mid[:, 1, :, :] = torch.clamp(tl_off_c_d_mid[:, 1, :, :], min=0.0)
        # br_off_c[:, 1, :, :] = torch.clamp(br_off_c[:, 1, :, :], min=0.0)
        tl_off_c2_mid = torch.zeros(tl_off_c_mid.size())
        # br_off_c2 = torch.zeros(tl_off_c.size())
        tl_off_c2_mid[:, 0, :, :] = torch.cos(tl_off_c_d_mid[:, 0, :, :] / 180 * np.pi) * tl_off_c_d_mid[:, 1, :, :]
        tl_off_c2_mid[:, 1, :, :] = -torch.sin(tl_off_c_d_mid[:, 0, :, :] / 180 * np.pi) * tl_off_c_d_mid[:, 1, :, :]
        # br_off_c2[:, 0, :, :] = torch.cos(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        # br_off_c2[:, 1, :, :] = -torch.sin(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        tl_off_c2_mid = tl_off_c2_mid.to("cuda")

        p2_offmap_mid = self.mid_p2_offset(tl_off_c2_mid.detach())  # 到可变形卷积的各个点
        x_p2_fadp_mid = self.mid_p2_fadp(p2_pool_mid, p2_offmap_mid)
        tl_det_mid = self.mid_tl_det(x_p2_fadp_mid)
        br_det_mid = self.mid_br_det(x_p2_fadp_mid)
        tl_off_mid = self.mid_tl_off(x_p2_fadp_mid)
        br_off_mid = self.mid_br_off(x_p2_fadp_mid)
        p2_off_c_2_mid = self.mid_p2_off_c_2(x_p2_fadp_mid)  # 矢量

        p1_pool_mid = x
        p1_heat_mid = self.p1_heat(x)
        # br_off_c = self.br_off_c(br_pool)
        p1_off_mid = self.p1_off(p1_pool_mid)
        p2_result_mid = torch.cat([p2_heat_mid, tl_off_c_mid, p2_off_c_2_mid, tl_det_mid, br_det_mid, tl_off_mid, br_off_mid], 1)
        p1_result_mid = torch.cat([p1_heat_mid, p1_off_mid], 1)
        # tl_pool_mid = x
        ...
        # tl_off_c_d_mid[:, 1, :, :] = torch.clamp(tl_off_c_d_mid[:, 1, :, :], min=0.0)
        # # br_off_c[:, 1, :, :] = torch.clamp(br_off_c[:, 1, :, :], min=0.0)
        # tl_off_c2_mid = torch.zeros(tl_off_c.size())
        # # br_off_c2 = torch.zeros(tl_off_c.size())
        # tl_off_c2_mid[:, 0, :, :] = torch.cos(tl_off_c_d_mid[:, 0, :, :] / 180 * np.pi) * tl_off_c_d_mid[:, 1, :, :]
        # tl_off_c2_mid[:, 1, :, :] = -torch.sin(tl_off_c_d_mid[:, 0, :, :] / 180 * np.pi) * tl_off_c_d_mid[:, 1, :, :]
        # # br_off_c2[:, 0, :, :] = torch.cos(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        # # br_off_c2[:, 1, :, :] = -torch.sin(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        # tl_off_c2_mid = tl_off_c2_mid.to("cuda")
        # tl_offmap_mid = self.mid_tl_offset(tl_off_c2_mid.detach())
        ...
        #
        #
        # br_off_c_d_mid = torch.zeros(br_off_c_mid.size())
        # br_off_c_d_mid[:, 0, :, :] = br_off_c_mid[:, 0, :, :] * 10
        # # br_off_c[:, 0, :, :] *= 10
        # br_off_c_d_mid[:, 1, :, :] = br_off_c_mid[:, 1, :, :]
        # # br_off_c[:, 1, :, :] *= 4
        # br_off_c_d_mid[:, 1, :, :] = torch.clamp(br_off_c_d_mid[:, 1, :, :], min=0.0)
        # # br_off_c[:, 1, :, :] = torch.clamp(br_off_c[:, 1, :, :], min=0.0)
        # br_off_c2_mid = torch.zeros(br_off_c.size())
        # # br_off_c2 = torch.zeros(tl_off_c.size())
        # br_off_c2_mid[:, 0, :, :] = torch.cos(br_off_c_d_mid[:, 0, :, :] / 180 * np.pi) * br_off_c_d_mid[:, 1, :, :]
        # br_off_c2_mid[:, 1, :, :] = -torch.sin(br_off_c_d_mid[:, 0, :, :] / 180 * np.pi) * br_off_c_d_mid[:, 1, :, :]
        # # br_off_c2[:, 0, :, :] = torch.cos(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        # # br_off_c2[:, 1, :, :] = -torch.sin(br_off_c[:, 0, :, :] / 180 * np.pi) * br_off_c[:, 1, :, :]
        # br_off_c2_mid = br_off_c2_mid.to("cuda")
        #
        # br_offmap_mid = self.mid_br_offset(br_off_c2_mid.detach())
        ...
        if self.with_mask:
            ...
            return p2_result, p1_result, None, p2_result_mid, p1_result_mid, None
    ...
    def loss(self, p2_result, p1_result, mask, p2_result_mid, p1_result_mid, mid_mask, gt_bboxes, gt_labels, gt_inflection, gt_masks,
             img_metas, cfg, imgscale):
        gt_p2_heatmap, gt_p1_heatmap, gt_tl_offsets, gt_br_offsets, gt_p1_offsets, gt_p2_off_c, \
        gt_p2_off_c2, gt_p2_tldet, gt_p2_brdet = corner_target(gt_bboxes=gt_bboxes, gt_labels=gt_labels, gt_inflection=gt_inflection, feats=p2_result,
                                                   imgscale=imgscale, direct=True, scale=1.0, dcn=True)
        # pred_tl_heatmap = _sigmoid(tl_result[:, :self.num_classes, :, :])
        pred_p2_heatmap = p2_result[:, :self.num_classes, :, :].sigmoid()
        pred_p2_off_c = p2_result[:, self.num_classes:self.num_classes + 2, :, :]
        pred_p2_off_c2 = p2_result[:, self.num_classes + 2:self.num_classes + 4, :, :]
        pred_p2_tldet = p2_result[:, self.num_classes + 4:self.num_classes + 5, :, :].sigmoid()
        pred_p2_brdet = p2_result[:, self.num_classes + 5:self.num_classes + 6, :, :].sigmoid()
        pred_p2_tloffsets = p2_result[:, -4:-2, :, :]
        pred_p2_broffsets = p2_result[:, -2:, :, :]
        # pred_br_heatmap = _sigmoid(br_result[:, :self.num_classes, :, :])
        pred_p1_heatmap = p1_result[:, :self.num_classes, :, :].sigmoid()
        ...
        pred_p1_offsets = p1_result[:, -2:, :, :]



        pred_p2_heatmap_mid = p2_result_mid[:, :self.num_classes, :, :].sigmoid()
        pred_p2_off_c_mid = p2_result_mid[:, self.num_classes:self.num_classes + 2, :, :]
        pred_p2_off_c2_mid = p2_result_mid[:, self.num_classes + 2:self.num_classes + 4, :, :]
        pred_p2_tldet_mid = p2_result_mid[:, self.num_classes + 4:self.num_classes + 5, :, :].sigmoid()
        pred_p2_brdet_mid = p2_result_mid[:, self.num_classes + 5:self.num_classes + 6, :, :].sigmoid()
        pred_p2_tloffsets_mid = p2_result_mid[:, -4:-2, :, :]
        pred_p2_broffsets_mid = p2_result_mid[:, -2:, :, :]
        # pred_br_heatmap = _sigmoid(br_result[:, :self.num_classes, :, :])
        pred_p1_heatmap_mid = p1_result_mid[:, :self.num_classes, :, :].sigmoid()
        # pred_br_off_c = br_result[:, self.num_classes:self.num_classes + 2, :, :]
        # pred_br_off_c2 = br_result[:, self.num_classes + 2:self.num_classes + 4, :, :]
        pred_p1_offsets_mid = p1_result_mid[:, -2:, :, :]
        ...
        p2_det_loss = det_loss_(pred_p2_heatmap, gt_p2_heatmap) + det_loss_(pred_p2_heatmap_mid, gt_p2_heatmap)
        p1_det_loss = det_loss_(pred_p1_heatmap, gt_p1_heatmap) + det_loss_(pred_p1_heatmap_mid, gt_p1_heatmap)
        ...
        l_det_loss = det_loss_(pred_p2_tldet, gt_p2_tldet)  + det_loss_(pred_p2_tldet_mid, gt_p2_tldet)
        r_det_loss = det_loss_(pred_p2_brdet, gt_p2_brdet) + det_loss_(pred_p2_brdet_mid, gt_p2_brdet)
        lr_det_loss = (l_det_loss + r_det_loss) / 2

        det_loss = (p2_det_loss + p1_det_loss) / 2
        # lr_det_loss = (l_det_loss + r_det_loss) / 2
        # f_det_loss = lr_det_loss + det_loss


        p2_off_mask = gt_p2_heatmap.eq(1).type_as(gt_p2_heatmap)
        br_off_mask = gt_p2_brdet.eq(1).type_as(gt_p2_brdet)
        tl_off_mask = gt_p2_tldet.eq(1).type_as(gt_p2_tldet)
        p1_off_mask = gt_p1_heatmap.eq(1).type_as(gt_p1_heatmap)
        # print(tl_off_mask)
        # print(">>>>>>>>>>>>>>>>>")
        # print(tl_off_mask.shape)
        # print("............")
        if p2_off_mask.sum() == 0:
            p2_num = 1
            r_num = 1
            l_num = 1
            p1_num = 1
        else:
            p2_num = p2_off_mask.sum()
            r_num = br_off_mask.sum()
            l_num = tl_off_mask.sum()
            p1_num = p1_off_mask.sum()
        p2_off_c_loss = off_loss_(pred_p2_off_c, gt_p2_off_c, mask=p2_off_mask)+ off_loss_(pred_p2_off_c_mid,
                                                                                            gt_p2_off_c,
                                                                                            mask=p2_off_mask)
        # br_off_c_loss = off_loss_(pred_br_off_c, gt_br_off_c, mask=br_off_mask) + off_loss_(mid_pred_br_off_c,
        #                                                                                     gt_br_off_c,
        #                                                                                     mask=br_off_mask)
        off_c_loss = p2_off_c_loss.sum() / p2_num
        off_c_loss /= 2.0
        off_c_loss *= 0.05

        p2_off_c2_loss = off_loss_(pred_p2_off_c2, gt_p2_off_c2, mask=p2_off_mask)  + off_loss_(pred_p2_off_c2_mid,
                                                                                               gt_p2_off_c2,
                                                                                               mask=p2_off_mask)
        # br_off_c2_loss = off_loss_(pred_br_off_c2, gt_br_off_c2, mask=br_off_mask) + off_loss_(mid_pred_br_off_c2,
        #                                                                                        gt_br_off_c2,
        #                                                                                        mask=br_off_mask)
        # print(gt_br_off_c2.sum(), gt_tl_off_c2.sum())
        off_c2_loss = p2_off_c2_loss.sum() / p2_num
        off_c2_loss /= 2.0
        off_c2_loss *= 0.5

        # print(off_c2_loss)

        tl_off_loss = off_loss_(pred_p2_tloffsets, gt_tl_offsets, mask=tl_off_mask)  + off_loss_(pred_p2_tloffsets_mid,
                                                                                              gt_tl_offsets,
                                                                                              mask=tl_off_mask)
        br_off_loss = off_loss_(pred_p2_broffsets, gt_br_offsets, mask=br_off_mask)  + off_loss_(pred_p2_broffsets_mid,
                                                                                              gt_br_offsets,
                                                                                              mask=br_off_mask)
        p1_off_loss = off_loss_(pred_p1_offsets, gt_p1_offsets, mask=p1_off_mask) + off_loss_(pred_p1_offsets_mid,
                                                                                              gt_p1_offsets,
                                                                                              mask=p1_off_mask)
        off_loss = tl_off_loss.sum() / l_num + br_off_loss.sum() / r_num + p1_off_loss.sum() / p1_num
        off_loss /= 2.0
        mask_loss = 0
        if self.with_mask:
        ...
        if self.with_mask:
            return dict(det_loss=det_loss, off_c_loss=off_c_loss+0.001, off_c2_loss=off_c2_loss+0.001, off_loss=off_loss+0.001,
                        mask_loss=mask_loss)
        else:
            return dict(lr_det_loss = lr_det_loss, det_loss=det_loss, off_c_loss=off_c_loss, off_c2_loss=off_c2_loss, off_loss=off_loss)
            # return dict(off_c_loss=off_c_loss, off_c2_loss=off_c2_loss, off_loss=off_loss)

    def inference_box(self, tl_result, br_result, img_metas, cfg, rescale=False):
    ...
    def inference(self, thersold, p2_result, p1_result, mask, mid_tl_result, mid_br_result, mid_mask, img_metas = [3, 512, 512],
                   rescale=False):
        num_classes = 1
        pred_p2_heatmap = p2_result[:, :num_classes, :, :].sigmoid()
        pred_p2_off_c = p2_result[:, num_classes:num_classes + 2, :, :]
        pred_p2_off_c2 = p2_result[:, num_classes + 2:num_classes + 4, :, :]
        pred_p2_tldet = p2_result[:, num_classes + 4:num_classes + 5, :, :].sigmoid()
        pred_p2_brdet = p2_result[:, num_classes + 5:num_classes + 6, :, :].sigmoid()
        pred_p2_tloffsets = p2_result[:, -4:-2, :, :]
        pred_p2_broffsets = p2_result[:, -2:, :, :]
        pred_p1_heatmap = p1_result[:, :num_classes, :, :].sigmoid()
        pred_p1_offsets = p1_result[:, -2:, :, :]
        # pdb.set_trace()
        # if len(tl_heat) == 2:
        ...
        batch_bboxes, batch_scores, boxes_xy = _decode_box(thersold, pred_p2_heatmap=pred_p2_heatmap, pred_p2_off_c=pred_p2_off_c,
                                                           pred_p2_off_c2=pred_p2_off_c2, pred_p2_tldet=pred_p2_tldet,
                                                           pred_p2_brdet=pred_p2_brdet, pred_p2_tloffsets=pred_p2_tloffsets,
                                                           pred_p2_broffsets=pred_p2_broffsets, pred_p1_heatmap=pred_p1_heatmap,
                                                           pred_p1_offsets=pred_p1_offsets,img_meta=img_metas)  # [0]
        # print(batch_bboxes)
        # print(batch_scores)
        for i in range(len(batch_bboxes)):
            # print(batch_bboxes[i].shape)
            if batch_bboxes[i].shape[0] != 0:
                index = nms_rotated(batch_bboxes[i], batch_scores[i], 0.4)
                batch_scores[i] = batch_scores[i].gather(0, index.long())
                # print(boxes_xy[i].shape)
                # print(index.unsqueeze(1).expand(-1, 8).long().shape)
                # boxes_xy = boxes_xy[i].gather(0, index.unsqueeze(1).expand(-1, 8).long())
                index = index.unsqueeze(1).expand(-1, 5).long()
                batch_bboxes[i] = batch_bboxes[i].gather(0, index.long())
            # print(boxes_xy)
